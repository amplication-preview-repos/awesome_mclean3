/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { OrderUpdateManyWithoutBotsInput } from "./OrderUpdateManyWithoutBotsInput";
import { Type } from "class-transformer";
import { ProductUpdateManyWithoutBotsInput } from "./ProductUpdateManyWithoutBotsInput";

@InputType()
class BotUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => OrderUpdateManyWithoutBotsInput,
  })
  @ValidateNested()
  @Type(() => OrderUpdateManyWithoutBotsInput)
  @IsOptional()
  @Field(() => OrderUpdateManyWithoutBotsInput, {
    nullable: true,
  })
  orders?: OrderUpdateManyWithoutBotsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  owner?: string | null;

  @ApiProperty({
    required: false,
    type: () => ProductUpdateManyWithoutBotsInput,
  })
  @ValidateNested()
  @Type(() => ProductUpdateManyWithoutBotsInput)
  @IsOptional()
  @Field(() => ProductUpdateManyWithoutBotsInput, {
    nullable: true,
  })
  products?: ProductUpdateManyWithoutBotsInput;
}

export { BotUpdateInput as BotUpdateInput };
